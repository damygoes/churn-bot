...
export type ButtonVariants = Array<NonNullable<VariantProps<typeof buttonVariants>["variant"]>>;

export type ButtonSizes = Array<NonNullable<VariantProps<typeof buttonVariants>["size"]>>;
...
const Button = forwardRef<HTMLButtonElement, ButtonProps>(
  (
    ...
    const renderIcon = (icon?: IconProps, margin?: string) =>
      icon ? (
        <span className={cn(children && margin, icon.name === "loading" && "animate-spin")}>
          <Icon {...icon} />
        </span>
      ) : null;
    ...
    return (
      ...
      >
        ...
        {iconPosition === "after" && !isLoading && renderIcon(effectiveIcon, "ml-1")}
      </Comp>
    );
  }
);
